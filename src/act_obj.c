...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	if ( str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
	    ...
	    get_obj( ch, obj, NULL );
	if (obj->pIndexData->vnum == OBJ_VNUM_OUTLAW
	...
	clan_shudder(obj->pIndexData->clan);
...
void do_put( CHAR_DATA *ch, char *argument )
{
    ...
    if (obj->pIndexData->vnum == OBJ_VNUM_CRUSADER
    || obj->pIndexData->vnum == OBJ_VNUM_ARCANA
...
void do_give( CHAR_DATA *ch, char *argument )
{
    ...
    enforcer_key_trigger(ch,victim,obj);
    outlaw_key_trigger(ch,victim,obj);
...
void do_empower( CHAR_DATA *ch, char *argument )
{
    ...
    chance = ch->pcdata->learned[gsn_empower];
...
void do_recite( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->clan == 4)
    {
	send_to_char("You are a Crusader, you don't use such things as scrolls!\n\r",ch);
	return;
    }
...
<mob> */
    ...
    if (victim->pIndexData->vnum == MOB_VNUM_LIFE)
    {
...
bool clan_down(CHAR_DATA *ch,int clan)
{
    OBJ_DATA *obj;
    bool is_down;
    ...
    {
	case (CLAN_ANCIENT):
	if (obj->in_room->vnum == ROOM_VNUM_LIFE
	|| obj->in_room->vnum == ROOM_VNUM_ENFORCER
	|| obj->in_room->vnum == ROOM_VNUM_CRUSADER
	|| obj->in_room->vnum == ROOM_VNUM_OUTLAW
	|| obj->in_room->vnum == ROOM_VNUM_ARCANA)
		is_down = TRUE;
	break;
	case (CLAN_LIFE):
	if (obj->in_room->vnum == ROOM_VNUM_ANCIENT
	|| obj->in_room->vnum == ROOM_VNUM_ENFORCER
	|| obj->in_room->vnum == ROOM_VNUM_CRUSADER
	|| obj->in_room->vnum == ROOM_VNUM_OUTLAW
	|| obj->in_room->vnum == ROOM_VNUM_ARCANA)
	...
	|| obj->in_room->vnum == ROOM_VNUM_CRUSADER
	|| obj->in_room->vnum == ROOM_VNUM_OUTLAW
	|| obj->in_room->vnum == ROOM_VNUM_ANCIENT)
		is_down = TRUE;
	break;
	case (CLAN_CRUSADER):
	if (obj->in_room->vnum == ROOM_VNUM_LIFE
    ...
    if (is_down)
	send_to_char("You can not find the Clan power within you.\n\r",ch);
...
/*
    ...
    if ((IS_SET(restrict,ANCIENT_ONLY) && ch->clan != CLAN_ANCIENT)
    || (IS_SET(restrict, ARCANA_ONLY) && ch->clan != CLAN_ARCANA)
    || (IS_SET(restrict, CRUSADER_ONLY) && ch->clan != CLAN_CRUSADER)
    || (IS_SET(restrict, ENFORCER_ONLY) && ch->clan != CLAN_ENFORCER)
    || (IS_SET(restrict, OUTLAW_ONLY) && ch->clan != CLAN_OUTLAW)
    || (IS_SET(restrict, LIFE_ONLY) && ch->clan != CLAN_LIFE)
    || (IS_SET(restrict, HUMAN_ONLY) && str_cmp(race,"human"))
    || (IS_SET(restrict, ELF_ONLY) && str_cmp(race,"elf"))
    || (IS_SET(restrict, GREY_ONLY) && str_cmp(race,"grey-elf"))
    || (IS_SET(restrict, DARK_ONLY) && str_cmp(race,"dark-elf"))
    || (IS_SET(restrict, GIANT_ONLY) && str_cmp(race,"giant"))
    ...
    || (IS_SET(restrict, DWARF_ONLY) && str_cmp(race,"dwarf"))
	...
	status = TRUE;
    if (IS_SET(restrict, CLERIC_ONLY) && ch->class != CLASS_CLERIC ) 
	status = TRUE;
    if (IS_SET(restrict, CHANNELER_ONLY) && ch->class != CLASS_CHANNELER ) 
	status = TRUE;
...
void wear_obj_girdle(CHAR_DATA *ch, OBJ_DATA *obj)  /* strange girdle */
{
    ...
    af.duration = -1;
    af.bitvector = 0;
    act("$n screams in agony as $s face contorts and twists!",ch,0,0,TO_ROOM);
...
void do_quest(CHAR_DATA *ch,char *argument)
{
    ...
    static const struct quest_data quest_info[] =
    {
	{" ",	0},
	{"The Dwarven Renegade",	2914},
	{"", 0}
    };

    for (questor = ch->in_room->people; questor != NULL; questor = ch->next_in_room)
    {
	if (!IS_NPC(questor))	continue;
	if (IS_SET(questor->act, ACT_QUESTOR))	break;
    }

    if (questor == NULL)
	{
	send_to_char("You can't do any quests from here.\n\r",ch);
	return;
   	}
     q_num = ch->pcdata->quest;
    one_argument(argument,arg);
    if (arg[0] == '\0')
    {
	if (q_num != 0)
	{
	sprintf(buf,"You are already on %s quest.\n\r",quest_info[q_num].name);
	send_to_char(buf,ch);
		return;
	}
	send_to_char("The following quests are available:\n\r",ch);
	for (i = 1; i < 2; i++)
	{
	sprintf(buf,"%d. %s quest.\n\r",i,quest_info[i].name);
	send_to_char(buf,ch);
	}
	return;
    }
