...
void save_char_obj( CHAR_DATA *ch )
{
	...
	fclose(fpReserve);
	if(ch->original_name) {
	    sprintf(strsave, "%s%s",GOD_DIR,capitalize(ch->original_name));
	} else {
	...
	fprintf(fp,"Lev %2d Trust %2d  %s%s\n",
	    ch->level, get_trust(ch), (ch->original_name) ? 
		ch->original_name : ch->name, ch->pcdata->title);
	fclose( fp );
...
/* kills and killed scores */
	fprintf( fp, "kls %d %d %d %d\n", 
		...
		ch->pcdata->kills[PK_LEVELS],
		ch->pcdata->kills[MOB_KILLS],
		ch->pcdata->kills[MOB_LEVELS]);

	fprintf( fp, "kld %d %d\n", 
	        ch->pcdata->kills[PK_KILLED],
		ch->pcdata->kills[MOB_KILLED]);
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'E':
		...
		if(ch->pcdata->extitle[0]=='\0') free_string(ch->pcdata->extitle);
                fMatch = TRUE;
                break;
            }
		...
		if(is_affected(ch,gsn_cloak_form)) {
			...
			ch->name=str_dup("cloaked figure");
		}
		return;
            }
	...
	case 'K':
		...
		ch->pcdata->kills[PK_LEVELS] = fread_number( fp );
		ch->pcdata->kills[MOB_KILLS] = fread_number( fp );
		ch->pcdata->kills[MOB_LEVELS] = fread_number( fp );
	     }
	    if (!str_cmp(word,"kld"))
 	    {
