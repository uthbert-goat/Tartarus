...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if(is_affected(ch,gsn_shadowstrike) && number_percent()<30) {
	...
	for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
   	{
		...
		if(vnum==MOB_VNUM_CRUSADER||vnum==MOB_VNUM_ARCANA||
			vnum==MOB_VNUM_LIFE||vnum==MOB_VNUM_ANCIENT||
			vnum==MOB_VNUM_ENFORCER||vnum==MOB_VNUM_OUTLAW)
		    continue;
	    }
...
void spell_venueport(int sn, int level,CHAR_DATA *ch,void *vo, int target)
{
    ...
    ||	 (victim->in_room->clan != 0 && victim->in_room->clan != CLAN_ARCANA)
    ||   ch->in_room->guild != 0
...
if (!(victim==NULL)) 
{
    ...
    ||   ch->in_room->guild != 0
    ||	 victim->in_room->guild != 0
    ||	 (victim->in_room->clan != 0 && victim->in_room->clan != ch->clan)
    ||   IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
    ||   IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL)
...
void spell_channel(int sn,int level, CHAR_DATA *ch,void *vo,int target)
{
    ...
    af.type = sn;
    af.duration = 24;
    af.modifier = ch->pcdata->perm_mana*75/100;
    af.bitvector = 0;
    af.level = level;
    affect_to_char(ch,&af);
    ch->hit += ch->pcdata->perm_mana*75/100;
...
void spell_prevent(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
	...
	if (ch->hit > ch->max_hit)
	{
	send_to_char("You are already overflwoing with health.\n\r",ch);
	return;
	}
        ...
        af.bitvector = 0;
        af.location = APPLY_HIT;
        af.modifier = (ch->pcdata->perm_hit+ch->pcdata->perm_mana)/2;
        affect_to_char(ch,&af);
	af.modifier = ch->level*3;
        af.location = APPLY_MOVE;
        affect_to_char(ch,&af);
	ch->hit += (ch->pcdata->perm_hit+ch->pcdata->perm_mana)/2;
...
void spell_guard_call(int sn,int level, CHAR_DATA *ch,void *vo,int target)
{
	...
	if (ch->in_room->clan == CLAN_OUTLAW)
	{
...
void spell_brew(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    if (obj->item_type != ITEM_TRASH && obj->item_type != ITEM_KEY)
    {
	send_to_char("You can't brew that type of item.\n\r",ch);
	return;
    }
    if (number_percent() > chance)
    {
	act("$p tries to brew a potion but fails.",ch,0,0,TO_ROOM);
	act("You fail and destroy $p without making a potion.",ch,obj,0,TO_CHAR);
	extract_obj(obj);
	check_improve(ch,sn,FALSE,1);
	return;
    }
    ...
    for (; ;)
    {
	gsn = number_range(1,MAX_SKILL);
	if (skill_table[gsn].skill_level[ch->class] > ch->level)
		continue;
	if (skill_table[gsn].slot == -1)
		continue;
	if (!str_cmp(skill_table[gsn].name, "heal")
	|| !str_cmp(skill_table[gsn].name, "cure critical")
	|| !str_cmp(skill_table[gsn].name, "cure serious")
	|| !str_cmp(skill_table[gsn].name, "cure poison")
	|| !str_cmp(skill_table[gsn].name, "cure disease")
	|| !str_cmp(skill_table[gsn].name, "detect invis")
	|| !str_cmp(skill_table[gsn].name, "detect magic")
	|| !str_cmp(skill_table[gsn].name, "fireball")
	|| !str_cmp(skill_table[gsn].name, "remove curse")
	|| !str_cmp(skill_table[gsn].name, "haste")
	|| !str_cmp(skill_table[gsn].name, "frenzy")
	|| !str_cmp(skill_table[gsn].name, "fly")
	|| !str_cmp(skill_table[gsn].name, "pass door"))
	{
		potion = create_object(get_obj_index(OBJ_VNUM_POTION),0);
		potion->level = level;
		act("$n brews a $p!",ch,potion,0,TO_ROOM);
		act("You brew a $p!",ch,potion,0,TO_CHAR);
		break;
	}
    }
...
void spell_team_spirit(int sn,int level, CHAR_DATA *ch,void *vo,int target)
{
    ...
    if (victim->clan != CLAN_ARCANA)
   {
	send_to_char("You can only team up with Clan Arcana members.\n\r",ch);
	return;
    }
...
void spell_decoy(int sn,int level, CHAR_DATA *ch,void *vo, int target)
{
    ...
    {
	send_to_char("The Godfather can't  figure out just who it is you're trying to decoy.\n\r",ch);
	return;
    }
...
void spell_alarm(int sn,int level,CHAR_DATA *ch,void *vo, int target)
{
    ...
    && pRoomIndex->clan != ch->clan)
    {
        send_to_char("You can't place alarms in an enemy Clan.\n\r",ch);
        return;
    }
...
void spell_power_word_fear(int sn, int level,CHAR_DATA *ch,void *vo,int target)
{
	...
	|| victim->pIndexData->vnum == MOB_VNUM_LIFE
...
void spell_undead_drain(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    if ( (victim->level + victim->drain_level) <= 1)
    {
    ...
    af.modifier = -(number_range(1,level/18));
    affect_join(victim,&af);
...
void spell_atrophy(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    if (IS_GOOD(ch))
    {
    send_to_char("Your God frowns upon your attempt to use this magic!\n\r",ch);
    return;
    }
