...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    enforcer_entry_trigger(ch);
    life_entry_trigger(ch);
    crusader_entry_trigger(ch);
    ancient_entry_trigger(ch);
    outlaw_entry_trigger(ch);
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    && ch->pcdata->death_status == HAS_DIED)
		return TRUE;
/* Make sure Clan guardians can always see */
    if (IS_NPC(ch))
    {
	if (ch->pIndexData->vnum == MOB_VNUM_ANCIENT
...
char * restrict_bit_name(restrict_flags)
{
    ...
    if ( restrict_flags & GREY_ONLY)	strcat( buf, " grey_only");
    ...
    if ( restrict_flags & CLERIC_ONLY)	strcat( buf, " cleric_only");
    ...
    if ( restrict_flags & CRUSADER_ONLY)	strcat( buf, " crusader_only");
    if ( restrict_flags & ANCIENT_ONLY)	strcat( buf, " ancient_only");
    if ( restrict_flags & MAGE_ONLY)	strcat( buf, " magic_users_only");
