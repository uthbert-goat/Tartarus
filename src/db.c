 ...
 */
#define                 MAX_STRING      3593120	/* Original was 1483120 */
...
void reset_area( AREA_DATA *pArea )
{
	...
	case 'O':
	    ...
	    obj->cost = 0;
	    obj->owner = "none";
		obj->talked = "someone a long long time ago";
	...
	if (obj->pIndexData->limtotal == 0)
		...
		if (pReset->arg3 == 16)
		{
			if ( (secondary = get_eq_char(mob,WEAR_WIELD)) != NULL)
			{
				unequip_char(mob,secondary);
				equip_char(mob,secondary,18);
				equip_char(mob,obj,16);
			}
			else
...
/* Assign clans to clan mobiles */
    if (pMobIndex->vnum == MOB_VNUM_ANCIENT)
	mob->clan = CLAN_ANCIENT;

    if (pMobIndex->vnum == MOB_VNUM_LIFE)
	mob->clan = CLAN_LIFE;

    if (pMobIndex->vnum == MOB_VNUM_ARCANA)
	mob->clan = CLAN_ARCANA;

    if (pMobIndex->vnum == MOB_VNUM_CRUSADER)
	mob->clan = CLAN_CRUSADER;

    if (pMobIndex->vnum == MOB_VNUM_ENFORCER_G)
	mob->clan = CLAN_ENFORCER;

    if (pMobIndex->vnum == MOB_VNUM_OUTLAW)
	mob->clan = CLAN_OUTLAW;
