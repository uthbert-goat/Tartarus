...
void do_leader( CHAR_DATA *ch, char *argument )
{
    ...
    if (victim->pcdata->induct == CLAN_LEADER)
    {
    ...
    else
    {
        ...
        send_to_char(buf, ch);
        victim->pcdata->induct = CLAN_LEADER;
        return;
    }
    return;
}
...
/* Induction in a Clan. When inducted a player gains skills or spells
specific for that Clan. You can add or remove abilities very easily by
...
void do_induct( CHAR_DATA *ch, char *argument )
{
    ...
    if ((ch->level < 56 && ch->pcdata->induct != CLAN_LEADER)
	|| IS_NPC(ch))
    {
        send_to_char("Huh?\n\r", ch);
        return;
    ...
    {
	buf[0] = '\0';
        send_to_char( "Syntax: induct <char> <clan name>\n\r",ch);
	send_to_char("Valid Clans are: ",ch);
	for (i = 1; i < (MAX_CLAN -1); i++)
	{
	sprintf(buf,"%s, ", clan_table[i].name);
	send_to_char(buf,ch);
	}
	sprintf(buf,"%s.\n\r",clan_table[MAX_CLAN -1].name);
	send_to_char(buf,ch);
        return;
    }
    if ( ( victim = get_char_world( ch, arg1 ) ) == NULL )
    ...
    if (IS_NPC(victim))
    {
	send_to_char( "Mobs can't be inducted into Clans.\n\r", ch );
	return;
    }

    if (!str_prefix(arg2,"none"))
    {
        if (ch->clan != victim->clan && ch->level < 56)
        {
	    ...
	    if (victim != ch)
	    {
            ...
            group_remove(victim,clan_table[victim->clan].name);
            clan =victim->clan;
        /* take away clan skills */
    if (clan == CLAN_ARCANA)
    {
	victim->pcdata->learned[skill_lookup("familiar")] = 0;
	victim->pcdata->learned[skill_lookup("brew")] = 0;
	victim->pcdata->learned[skill_lookup("spiritblade")] = 0;
	victim->pcdata->learned[skill_lookup("venueport")] = 0;
	victim->pcdata->learned[skill_lookup("team spirit")] = 0;
    	victim->pcdata->learned[skill_lookup("channel")] = 0;
    }	    
    else if (clan == CLAN_ANCIENT)
    {
        victim->pcdata->learned[gsn_cloak_form] = 0;
	victim->pcdata->learned[gsn_eye_of_the_predator] = 0;
	victim->pcdata->learned[gsn_shadowgate] = 0;
    }
    else if (clan == CLAN_LIFE)
    {
	victim->pcdata->learned[skill_lookup("lifeline")] = 0;
	victim->pcdata->learned[skill_lookup("safety")] = 0;
	victim->pcdata->learned[skill_lookup("sunbolt")] = 0;
	victim->pcdata->learned[gsn_aura_of_sustenance] = 0;
	victim->pcdata->learned[skill_lookup("prevent")] = 0;
	victim->pcdata->learned[skill_lookup("life transfer")] = 0;
	victim->pcdata->learned[skill_lookup("lightshield")] = 0;    
    }
    else if (clan == CLAN_CRUSADER)
    {
	victim->pcdata->learned[gsn_bandage] = 0;
	victim->pcdata->learned[gsn_spellbane] = 0;
	victim->pcdata->learned[gsn_trophy] = 0;
	victim->pcdata->learned[gsn_counter_parry] = 0;
	victim->pcdata->learned[gsn_vigilance] = 0;
	victim->pcdata->learned[gsn_steel_nerves] = 0;
	victim->pcdata->learned[gsn_battlecry] = 0;
    }
    else if (clan == CLAN_ENFORCER)
    {
 	victim->pcdata->learned[skill_lookup("judgement")] = 0;
 	victim->pcdata->learned[skill_lookup("trace")] = 0;
	victim->pcdata->learned[gsn_true_sight] = 0;
	victim->pcdata->learned[skill_lookup("hand of vengeance")] = 0;
	victim->pcdata->learned[gsn_wanted] = 0;
	victim->pcdata->learned[skill_lookup("guard call")] = 0;
	victim->pcdata->learned[skill_lookup("protectorate plate")] = 0;
    }
    else /* clan == CLAN_OUTLAW */
    {
	victim->pcdata->learned[skill_lookup("decoy")] = 0;
	victim->pcdata->learned[skill_lookup("badge")] = 0;
	victim->pcdata->learned[gsn_revolt] = 0;
	victim->pcdata->learned[gsn_vanish] = 0;
	victim->pcdata->learned[skill_lookup("riot")] = 0;    
    }
            victim->clan = 0;
            return;
        }
    ...
    if ((clan = clan_lookup(arg2)) != ch->clan &&
        ch->level < 56)
    {
        send_to_char("You may only induct into the clan which you belong.\n\r", ch);
        return;
    }
    ...
    group_remove(victim,clan_table[victim->clan].name);
    victim->clan = clan;
    group_add(victim,clan_table[clan].name,FALSE);
    if (clan == CLAN_ANCIENT)
    {
        victim->pcdata->learned[gsn_cloak_form] = 75;
        victim->pcdata->learned[gsn_eye_of_the_predator] = 75;
        victim->pcdata->learned[gsn_shadowgate] = 75;
    }
    else if (clan == CLAN_LIFE)
    {
	victim->pcdata->learned[skill_lookup("lifeline")] = 75;
	victim->pcdata->learned[skill_lookup("safety")] = 75;
	victim->pcdata->learned[skill_lookup("sunbolt")] = 75;
	victim->pcdata->learned[gsn_aura_of_sustenance] = 75;
	victim->pcdata->learned[skill_lookup("prevent")] = 75;
	victim->pcdata->learned[skill_lookup("life transfer")] = 75;
	victim->pcdata->learned[skill_lookup("lightshield")] = 75;
    }
    else if (clan == CLAN_ARCANA)
    {
	victim->pcdata->learned[skill_lookup("familiar")] = 75;
	victim->pcdata->learned[skill_lookup("brew")] = 75;
	victim->pcdata->learned[skill_lookup("spiritblade")] = 75;
	victim->pcdata->learned[skill_lookup("venueport")] = 75;
	victim->pcdata->learned[skill_lookup("team spirit")] = 75;
    	victim->pcdata->learned[skill_lookup("channel")] = 75;

    }
    else if (clan == CLAN_CRUSADER)
    {
	victim->pcdata->learned[gsn_bandage] = 75;
	victim->pcdata->learned[gsn_spellbane] = 75;
	victim->pcdata->learned[gsn_trophy] = 75;
	victim->pcdata->learned[gsn_vigilance] = 75;
	victim->pcdata->learned[gsn_battlecry] = 75;
	victim->pcdata->learned[gsn_steel_nerves] = 75;
    }
    else if (clan == CLAN_ENFORCER)
    {
 	victim->pcdata->learned[skill_lookup("judgement")] = 75;
 	victim->pcdata->learned[skill_lookup("trace")] = 75;
	victim->pcdata->learned[gsn_true_sight] = 75;
	victim->pcdata->learned[skill_lookup("hand of vengeance")] = 75;
	victim->pcdata->learned[gsn_wanted] = 100;
	victim->pcdata->learned[skill_lookup("guard call")] = 75;
	victim->pcdata->learned[skill_lookup("protectorate plate")] = 75;
    }
    else /* clan == CLAN_OUTLAW */
    {
	victim->pcdata->learned[skill_lookup("decoy")] = 75;
	victim->pcdata->learned[skill_lookup("badge")] = 75;
	victim->pcdata->learned[gsn_revolt] = 75;
	victim->pcdata->learned[gsn_vanish] = 75;
	victim->pcdata->learned[skill_lookup("riot")] = 75;    
    }
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf,"Hometown: %s   Specialization: %d\n\r",
(victim->temple == TEMPLE_NEW_THALOS) ? "New thalos" :
(victim->temple == TEMPLE_OFCOL ) ? "Ofcol" :
(victim->temple == TEMPLE_ARKHAM ) ? "Arkham" :
"Midgaard",
IS_NPC(ch) ? 0 : ch->pcdata->special);
    send_to_char(buf,ch);
    ...
    {
	sprintf( buf,
	    "Thirst: %d  Hunger: %d  Full: %d  Drunk: %d  Starve: %d  Dehydrate: %d\n\r",
    ...
    {
    	sprintf( buf, 
	    "Age: %d  Played: %d  Last Level: %d  Timer: %d  SP: %d.\n\r",
	    get_age(victim), 
	    (int) (victim->played + current_time - victim->logon) / 3600, 
	    victim->pcdata->last_level, 
	    victim->timer, victim->pcdata->sp );
    	send_to_char( buf, ch );
	sprintf(buf,
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("    home xptotal home\n\r",ch);
	return;
    }
    ...
    if ( arg2[0] == '\0' || !str_cmp( arg2, "home"))
    {
	if (value != TEMPLE_MIDGAARD
	&& value != TEMPLE_OFCOL
	&& value != TEMPLE_NEW_THALOS
	&& value != TEMPLE_ARKHAM)
	{
sprintf(buf,"Midgaard (any): %d\n\r",TEMPLE_MIDGAARD);
send_to_char(buf,ch);
sprintf(buf,"Ofcol (goods): %d\n\r",TEMPLE_OFCOL);
send_to_char(buf,ch);
sprintf(buf,"New Thalos (neutrals): %d\n\r",TEMPLE_NEW_THALOS);
send_to_char(buf,ch);
sprintf(buf,"Arkham (evils): %d\n\r",TEMPLE_ARKHAM);
send_to_char(buf,ch);
...
hostnames. 
 */
	    sprintf( buf + strlen(buf), "[%3d %14s] %s@%s\n\r",
		d->descriptor,
	...
	d->connected == (CON_READ_MOTD) ? "Getting motd" : "null",
		d->original  ? (d->original->original_name) ?
		d->original->original_name : d->original->name  :
		d->character ? (d->character->original_name) ?
		d->character->original_name : d->character->name : "(none)",
	(ch->level >= 55) ? d->host : "unknown"
		);
	}
    }
    if (count == 0)
